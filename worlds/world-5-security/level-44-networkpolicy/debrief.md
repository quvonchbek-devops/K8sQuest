...
**Natija:** Backend connects successfully, DNS works, security maintained
...
**Always include** DNS egress or pods hal qila olmaydi hostnames!
...
```yaml
# ❌ Backend hal qila olmaydi database hostname
egress:
- to:
  - podSelector:
      matchLabels:
...
```yaml
# ❌ Using Service port o'rniga of container port
ports:
...
**Kompaniya:** Cryptocurrency exchange  
**Date:** June 2023  
**Ta'sir:** $4.2M stolen, complete platform compromise
...
**With these policies:**
- Web pod ulana olmadi to database → attack stopped
- Web pod ulana olmadi to wallet → theft to'sib qo'ydi
...
```bash
# Tekshirish pod has expected labels
kubectl get pod my-pod --show-labels

# Policy won't apply if labels mos kelmayapti
```
...
**Excellent work!** Siz o'zlashtirgansiz Kubernetes network security! 🎉🔒
